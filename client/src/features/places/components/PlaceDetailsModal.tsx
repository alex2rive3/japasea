...
export const PlaceDetailsModal: React.FC<PlaceDetailsModalProps> = ({ 
  ...
  const { selectedPlace, isLoading, fetchPlaceById, ensurePlace } = usePlaces()
  ...
  useEffect(() => {
    const fetchPlaceDetails = async () => {
      if (place && open) {
        try {
          // Si el lugar ya tiene todos los datos, no necesitamos hacer nada más
          if (place.images && place.openingHours) {
            return
          }
          
          // Si tiene un ID válido, obtener detalles completos
          ...
          if (isValidObjectId) {
            await fetchPlaceById(candidateId)
          } else {
    ...
    fetchPlaceDetails()
  }, [place, open, fetchPlaceById, ensurePlace])
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: displayPlace.name || displayPlace.key,
        text: t('places.shareMessage', { name: displayPlace.name || displayPlace.key }),
  ...
  const getDefaultImage = (type: string) => {
    const defaultImages: { [key: string]: string } = {
      ...
      'breakfast and snacks': 'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=800&q=80',
  ...
  const displayImages = displayPlace.images && displayPlace.images.length > 0
    ? displayPlace.images
    : [{ url: getDefaultImage(displayPlace.type || 'default'), caption: displayPlace.name || displayPlace.key }]
    ...
    <>
        ...
        <DialogTitle sx={{ m: 0, p: 2 }}>
          <Stack direction="row" alignItems="center" justifyContent="space-between">
            <Typography variant="h5" component="div" fontWeight={600}>
              {displayPlace.name || displayPlace.key}
            </Typography>
            <IconButton
              aria-label="close"
              onClick={onClose}
            ...
            <>
              ...
              <Box sx={{ position: 'relative', height: 200, overflow: 'hidden' }}>
                <Grid container spacing={0.5} sx={{ height: '100%' }}>
                  {displayImages.slice(0, 4).map((image, index) => (
                    <Grid 
                      key={index} 
                      size={displayImages.length === 1 ? 12 : 6} 
                      sx={{ height: displayImages.length === 1 ? '100%' : '50%' }}
                    >
                      <Card sx={{ height: '100%', borderRadius: 0 }}>
                        <CardMedia
                          component="img"
                          height="100%"
                          image={imageError[`img-${index}`] ? getDefaultImage(displayPlace.type || 'default') : image.url || image} 
                          alt={typeof image === 'string' ? displayPlace.name || displayPlace.key : image.caption || displayPlace.name || displayPlace.key}
              ...
              <Box sx={{ p: 3 }}>
                ...
                <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 2 }}>
                  <Chip 
                    label={displayPlace.type || t('places.place')} 
                    color="primary" 
                    size="small"
                  ...
                  {displayPlace.rating && (
                    <Stack direction="row" alignItems="center" spacing={0.5}>
                      <Rating 
                        value={typeof displayPlace.rating === 'number' ? displayPlace.rating : displayPlace.rating.average} 
                        readOnly 
                        size="small" 
                        precision={0.5}
                      />
                      <Typography variant="body2" color="text.secondary">
                        {typeof displayPlace.rating === 'object' && displayPlace.rating.count
                  ...
                  <>
                    ...
                    <Stack direction="row" spacing={1} alignItems="flex-start">
                      ...
                      <Box>
                        ...
                        <Stack spacing={0.5}>
                          {typeof displayPlace.openingHours === 'string' ? (
                ...
                <Stack direction="row" spacing={2} sx={{ mt: 3, alignItems: 'center' }}>
                  ...
                  {placeId && placeId.length > 0 ? (
                    <FavoriteButton 
                      placeId={placeId}
                      placeName={displayPlace.name || displayPlace.key}
                      showTooltip={false}
                      size="medium"
                    />
                  ) : (
      ...
      {placeId && placeId.length > 0 && (
        <ReviewForm
          ...
          placeId={placeId}
          placeName={displayPlace.name || displayPlace.key || t('places.thisPlace')}
